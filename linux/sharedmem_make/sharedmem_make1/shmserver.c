...
int main()
{
	...
	if((shmserver = (struct shmbuf *)shmat(shmid, 0, 0)) == (struct shmbuf *)-1)
	{
		perror("shmat");
		exit(1);
	}
	...
	printf("Writing data into the shared memory as follows:\n");
        shmserver->age = 100;
        strcpy(shmserver->data, "Bhaskar is a good boy");
	...
	printf("Maximum number of segments that a process can attach; unused with in kernel:%ld\n", buf2.shmseg);
	printf("Maximum number of pages of shared memory, system-wise	:ld\n", buf2.shmall);
